    ...
    public function actionRegister()
    {
        ...
        if ($user->load($post)) {
            ...
            if ($user->validate() && $profile->validate()) {
                ...
                $successText = Yii::t("user", "Registro realizado com sucesso ({displayName})", ["displayName" => $user->getDisplayName()]);
                $guestText = "";
                if (Yii::$app->user->isGuest) {
                    $guestText = Yii::t("user", " - Please check your email to confirm your account");
                }
                Yii::$app->session->setFlash("Register-success", $successText . $guestText . ". Use o formulário abaixo para enviar sua mensagem");
                return $this->redirect(['/occurrence/create']);
            }
        }
